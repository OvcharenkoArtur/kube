...
```shell
docker login home.local
```

```shell
docker build -t home.local/library/ubuntu_ssh:24.04 .
```

```shell
docker push home.local/library/ubuntu_ssh:24.04
```
...
```shell
git clone https://gitlab.home.local/dev/charts/devcontainer.git
```
...
```shell
helm install artur oci://home.local/charts/devcontainer --version 0.1.0
```
...
```shell
ssh artur@192.168.0.113 -p 31022
```
